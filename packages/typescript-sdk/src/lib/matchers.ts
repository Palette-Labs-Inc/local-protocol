...
import { LocalProtocolDefaultError } from "../models/errors/local-protocol-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | LocalProtocolDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | LocalProtocolDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new LocalProtocolDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new LocalProtocolDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
