...
import { PostalAddress, PaymentCredential, PaymentInstrument, Payment, paymentJsonSchema, paymentInstrumentJsonSchema, } from "./schemas/ucp.js";
import { Coordinates, Location, locationJsonSchema, DeliveryRequest, DeliveryQuote, Delivery, } from "./schemas/delivery.js";
import { DeliveryEventVocabulary, deliveryEventsJsonSchema, courierVocabularyData, } from "./schemas/delivery-events.js";
...
import { Interval, Availability, ModifierItem, ModifierGroup, ModifierOption, CatalogItem, CatalogCategory, Catalog, Merchant, } from "./schemas/catalog.js";
...
function getSchemasDir() {
    if (fs.existsSync(SCHEMAS_DIR))
        return SCHEMAS_DIR;
    if (fs.existsSync(PACKAGE_SCHEMAS_DIR))
...
function sortKeys(obj, keyOrder) {
    const result = {};
...
function removeDateTimePatterns(obj) {
    ...
    const o = obj;
    if (o.format === "date-time" && "pattern" in o)
        delete o.pattern;
    for (const v of Object.values(o))
        removeDateTimePatterns(v);
}
...
function normalizeZodOutput(obj) {
    if (obj == null || typeof obj !== "object")
        return;
    if (Array.isArray(obj)) {
    ...
    const o = obj;
    if (o.additionalProperties && typeof o.additionalProperties === "object" && Object.keys(o.additionalProperties).length === 0) {
        o.additionalProperties = true;
    }
    if (o.type === "integer" && o.maximum === SAFE_INTEGER_MAX) {
        delete o.maximum;
    }
    for (const v of Object.values(o))
        normalizeZodOutput(v);
}
/** Replace anyOf with oneOf only where the original uses oneOf (e.g. amount currency) */
function oneOfWhereOriginal(schema, id) {
...
function reorderSchema(schema, id) {
    const rootOrder = id.startsWith("shared/") || id.startsWith("catalog/") || id.startsWith("payment/")
    ...
    if (out.properties && typeof out.properties === "object") {
        ...
        for (const [k, v] of Object.entries(props)) {
            reordered[k] = typeof v === "object" && v !== null && !Array.isArray(v)
                ? sortKeys(v, PROP_KEY_ORDER)
                : v;
        }
        out.properties = reordered;
    }
    if (Array.isArray(out.anyOf)) {
        out.anyOf = out.anyOf.map((item) => typeof item === "object" && item !== null && !Array.isArray(item)
            ? reorderSchema(item, "")
            : item);
    }
    if (Array.isArray(out.oneOf)) {
        out.oneOf = out.oneOf.map((item) => typeof item === "object" && item !== null && !Array.isArray(item)
            ? reorderSchema(item, "")
            : item);
    }
    if (Array.isArray(out.allOf)) {
        out.allOf = out.allOf.map((item) => typeof item === "object" && item !== null && !Array.isArray(item)
            ? reorderSchema(item, id)
            : item);
    }
    return out;
}
...
function normalizeSchema(schema, id) {
    const copy = JSON.parse(JSON.stringify(schema));
    delete copy.id;
    if (!SCHEMAS_WITH_$ID.has(id))
        delete copy.$id;
...
function reorderToMatch(original, generated) {
    ...
    if (Array.isArray(original)) {
        ...
        return original.map((origItem, i) => i < genArr.length ? reorderToMatch(origItem, genArr[i]) : origItem);
    }
...
function relativizeRefInSchema(schema, fromPath, allIds, outDir) {
    function walk(obj) {
        if (obj == null || typeof obj !== "object")
            return;
        if (Array.isArray(obj)) {
...
function main() {
    ...
    for (const [id, schema] of Object.entries(schemas)) {
        ...
        if (fs.existsSync(originalPath)) {
            ...
            toWrite = reorderToMatch(original, toWrite);
        }
    ...
    fs.writeFileSync(courierDest, JSON.stringify(courierVocabularyData(), null, 2) + "\n", "utf8");
