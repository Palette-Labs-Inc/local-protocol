...
class DeliveryEventTest(IntegrationTestBase):
  ...
  def test_event_vocabulary_includes_version(self) -> None:
    ...
    self.assertIn(
      "@",
      delivery.event_vocabulary,
      f"event_vocabulary missing version: {delivery.event_vocabulary}",
    )

  def test_event_vocabulary_version_is_date_format(self) -> None:
    """event_vocabulary version MUST be valid YYYY-MM-DD format."""
    delivery = self._create_delivery_sdk()
    parts = delivery.event_vocabulary.split("@")
    self.assertEqual(
      len(parts),
      2,
      f"Invalid event_vocabulary format: {delivery.event_vocabulary}",
    )
    self.assertRegex(
      parts[1],
      r"^\d{4}-\d{2}-\d{2}$",
      f"Invalid date version in event_vocabulary: {delivery.event_vocabulary}",
    )
  ...
  def test_event_exists_in_vocabulary(self) -> None:
    ...
    # Extract standard name from reference
  ...
  def test_updated_at_is_iso8601(self) -> None:
    ...
    delivery = self._create_delivery_sdk()
    # SDK already parses to datetime, so it's valid
    self.assertIsInstance(delivery.updated_at, datetime)
  ...
  def test_get_delivery_by_id(self) -> None:
    ...
    delivery = self._create_delivery_sdk()
    fetched = self.sdk.deliveries.retrieve(delivery.id)
