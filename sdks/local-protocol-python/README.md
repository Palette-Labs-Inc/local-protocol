# Local Protocol Python API library

<!-- prettier-ignore -->
[![PyPI version](https://img.shields.io/pypi/v/local_protocol.svg?label=pypi%20(stable))](https://pypi.org/project/local_protocol/)

The Local Protocol Python library provides convenient access to the Local Protocol REST API from any Python 3.9+
...
> [!NOTE]
> Once this package is [published to PyPI](https://www.stainless.com/docs/guides/publish), this will become: `pip install local_protocol`
...
delivery_request = client.requests.create(
    id="id",
    dropoff_location={},
    dropoff_time=datetime.fromisoformat("2019-12-27T18:11:19.117"),
    nonce="nonce",
    pickup_location={},
    pickup_time=datetime.fromisoformat("2019-12-27T18:11:19.117"),
)
print(delivery_request.dropoff_location)
```

## Handling errors

When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `local_protocol.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `local_protocol.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `local_protocol.APIError`.

```python
import local_protocol
from local_protocol import LocalProtocol

client = LocalProtocol()

try:
    client.well_known.retrieve()
except local_protocol.APIConnectionError as e:
    print("The server could not be reached")
    print(e.__cause__)  # an underlying Exception, likely raised within httpx.
except local_protocol.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except local_protocol.APIStatusError as e:
...
client.with_options(max_retries=5).well_known.retrieve()
```
...
client.with_options(timeout=5.0).well_known.retrieve()
```
...
You can enable logging by setting the environment variable `LOCAL_PROTOCOL_LOG` to `info`.

```shell
$ export LOCAL_PROTOCOL_LOG=info
```
...
```python
import httpx
from local_protocol import LocalProtocol, DefaultHttpxClient

client = LocalProtocol(
    # Or use the `LOCAL_PROTOCOL_BASE_URL` env var
...
```py
import local_protocol
print(local_protocol.__version__)
```
