...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('default logLevel is warn', async () => {
      const client = new LocalProtocol({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new LocalProtocol({ logger: logger, apiKey: 'My API Key' });
  ...
  describe('baseUrl', () => {
    ...
    test('explicit option', () => {
      const client = new LocalProtocol({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new LocalProtocol({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new LocalProtocol({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new LocalProtocol({ apiKey: 'My API Key' });
  ...
  test('with overridden environment variable arguments', () => {
    ...
    process.env['LOCAL_PROTOCOL_API_KEY'] = 'another My API Key';
    const client = new LocalProtocol({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new LocalProtocol({ apiKey: 'My API Key', fetch: testFetch });
