# Local Protocol TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/local-protocol.svg?label=npm%20(stable)>)](https://npmjs.org/package/local-protocol) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/local-protocol)
...
> [!NOTE]
> Once this package is [published to npm](https://www.stainless.com/docs/guides/publish), this will become: `npm install local-protocol`
...
<!-- prettier-ignore -->
```ts
const wellKnown = await client.wellKnown.retrieve().catch(async (err) => {
  if (err instanceof LocalProtocol.APIError) {
...
await client.wellKnown.retrieve({
  maxRetries: 5,
});
```
...
const response = await client.wellKnown.retrieve().asResponse();
...
1. Via the `LOCAL_PROTOCOL_LOG` environment variable
...
const client = new LocalProtocol({
  logger: logger.child({ name: 'LocalProtocol' }),
...
const client = new LocalProtocol({ fetch });
```
...
```ts
import LocalProtocol from 'npm:local-protocol';
